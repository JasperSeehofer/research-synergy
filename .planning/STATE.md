...
status: in_progress
stopped_at: "EXP-RS-17 (Phase 36) RUN 2026-07-07 — MethMeSH (mechanism-ontology #4) KILLED-BY-CONSTRUCTION at the cheap tagging-recall gate: Feynman 1/5 (FAIL, gate 3/5), modern 4/6 (PASS) → P2 falsified → pre-registered KILL, reached before any distractor tagging/full eval. Mechanism = exact-archetype-ID granularity brittleness (analogous papers get neighboring non-identical archetypes). Full record: .planning/phases/36-methmesh-vs-baseline/36-VERIFICATION.md. **EXP-RS-19 (Phase 38) RUN 2026-07-07 (ultracode) — HyDE-Bridge: pinned headline KILLED at the cheap gate, but the generation MECHANISM is PROVEN.** 21-agent judge-panel design. Cheap Feynman gate (C-34): headline (K=5,max,λ=0) forward recall@10 = 0.20 < 0.40 null → GATE-A FAILS → KILL. BUT GATE-B PASSES: pair04 (percolation→epidemics) recovered rank 17→4, VERIFIED real (blind epidemiology hypothetical matches the real epidemics paper, cos 0.15 vs null 0.06 via transmission/epidemics tokens). Cause of KILL = max-pool distractor inflation, monotonic in K (K1=0.60, K3=0.40, K5=0.20); the pre-registered K=1 ablation TIES the LLM baseline 0.60 (same pairs 03/04/05) but is non-promotable (C-36 pin). Signal survives → violates C-37's 'no signal' KILL premise. Full record: .planning/phases/38-hyde-bridge/38-VERIFICATION.md. **AWAITING HUMAN go/kill/pivot:** (rec.) build the **generate→VERIFY cascade (#3)** — HyDE is the proven recall stage that casts the cross-field net, an LLM/CAS verify stage prunes the max-pool distractor inflation (both C-37's escalation branch and the design's PIVOT converge here); OR escalate to a real semantic-embedding substrate; OR a fresh EXP-RS-20 with distractor-robust aggregation (K=1 / rank-fusion). Chapter status: SME (16), mechanism-ontology exact+soft (17/18) all KILLED; HyDE (19) = first PARTIAL signal (recovers a null-missed pair, ties LLM at K=1). Bar = brute-force LLM recall@10 0.60."
