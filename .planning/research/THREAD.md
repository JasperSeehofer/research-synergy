...
discovery), decoupled from any specific graph-dynamical method. New direction is being generated by
a fan-out ideation workflow (see `.planning/research/BRAINSTORM-cross-field-transfer.md` when it lands).
The hard core "bridges emerge from graph dynamics" is abandoned; the working baseline reverts to the
brute-force LLM community-pair comparison (EXP-RS-10) until a new direction is chosen.
