...
# --- Paths ---
DATA_FILE      = 'data/research_synergy_bridged_fine.json'
...
# ==========================================================================
# v07 (EXP-RS-14): SYNC-AWARE K selection. v06 confound: compute_K_stable's
# lambda_2>=0 criterion locked onto a low-K SCATTERED fixed point (r=0.136) on the
# dense graph. Fix: pick the minimum K at which the system ACTUALLY synchronizes
# (r_global >= 0.90). Principled well-posedness fix (like giant-CC), NOT benchmark tuning.
# ==========================================================================
def _probe_r(K, A_csr, omega, theta0, D_inv_arr, t_end=150):
    def _rhs(t, th):
        s, c = np.sin(th), np.cos(th)
        coup = (A_csr @ s) * c - (A_csr @ c) * s
        return omega + K * D_inv_arr * coup
    sol = solve_ivp(_rhs, (0, t_end), theta0, max_step=1.0, rtol=1e-4, atol=1e-6, method='RK45')
    return float(abs(np.mean(np.exp(1j * sol.y[:, -1]))))

def find_K_sync(A_csr, omega, theta0, r_target=0.90):
    deg = np.array(A_csr.sum(axis=1)).flatten()
    D_inv_arr = 1.0 / np.maximum(deg, 1e-12)
    trace, K_sync = [], None
    for K in [1, 2, 4, 8, 12, 16, 24, 32, 48, 64]:
        r = _probe_r(K, A_csr, omega, theta0, D_inv_arr)
        trace.append((K, r)); print(f'  K={K:>4}: r_global={r:.3f}')
        if r >= r_target: K_sync = K; break
    if K_sync is None:
        K_sync = trace[-1][0]; print(f'  [warn] r_target not reached; using K={K_sync}')
    print(f'  -> K_sync = {K_sync} (min K with r_global >= {r_target})')
    return float(K_sync), trace
...
print('Finding K_sync (min K with r_global >= 0.90) ...')
t_start = time.time()
K_stable, bisect_trace = find_K_sync(A, omega_corp, theta0_fixed, r_target=0.90)
...
ax.axis('off')
plt.tight_layout()
plt.savefig('kuramoto_v07_bridges.png', dpi=120, bbox_inches='tight')
plt.show()
print('Figure saved: kuramoto_v07_bridges.png')
...
# ============================================================================
print('=' * 80)
print('KURAMOTO-LBD v0.7 (EXP-RS-14) — RESULTS SUMMARY')
print('=' * 80)
...
# ==========================================================================
# v07 per-pair recall metric (EXP-RS-14)
# ==========================================================================
def _perpair(bridges_list):
    rows = []
    for r in evaluable:
        ac, bc = r.get('a_comm'), r.get('b_comm'); best = None
    ...
    return rows
_pp = _perpair(bridges_corp[MID_K])
PERPAIR_RECALL_AT10 = sum(1 for *_, rk in _pp if rk and rk <= 10) / max(len(_pp), 1)
PERPAIR_RECALL_ANY  = sum(1 for *_, rk in _pp if rk) / max(len(_pp), 1)
print(f'\nSync sweep: {bisect_trace}')
print('PER-PAIR bridge ranks (first top-200 bridge spanning the pair community-pair):')
for pid, ac, bc, rk in _pp:
    print(f'  {pid:26} comm {ac}-{bc}:  rank {rk if rk else ">200"}')
print(f'Per-pair recall@10 = {PERPAIR_RECALL_AT10:.3f}   recall@any(top200) = {PERPAIR_RECALL_ANY:.3f}   global BENCH_P10 = {BENCH_P10:.3f}')
...
_results = {
  ...
  'r_sweep': [[float(k),float(r)] for k,r in bisect_trace],
...
with open('data/kuramoto_v07_results.json', 'w') as _f:
    _json.dump(_results, _f, indent=1)
print('\n[v05] results -> data/kuramoto_v07_results.json')
