...
def sheaf_perpair(bridges, corpus_nodes, gt_path):
    """Per-pair recall@10 + full-list rank of each benchmark community-pair (mirrors kuramoto v07)."""
    with open(gt_path) as f:
        gt = json.load(f)
    evaluable = set(gt.get("evaluable_pairs", []))
    ranked = [(min(c1, c2), max(c1, c2)) for (c1, c2), _, _ in bridges]
    rows = []
    for pair in gt["pairs"]:
        pid = pair["id"]
        if not any(pid.startswith(ep) for ep in evaluable):
            continue
        sa = (pair["side_a"].get("arxiv_id") or "").split("v")[0]
        sb = (pair["side_b"].get("arxiv_id") or "").split("v")[0]
        ca, cb = corpus_nodes.get(sa), corpus_nodes.get(sb)
        if ca is None or cb is None or ca == cb:
            rows.append((pid, ca, cb, None)); continue
        cp = (min(ca, cb), max(ca, cb))
        rank = ranked.index(cp) + 1 if cp in ranked else None
        rows.append((pid, ca, cb, rank))
    n = max(sum(1 for *_, rk in rows), 1)
    recall = sum(1 for *_, rk in rows if rk and rk <= 10) / n
    return recall, rows
...
def main():
    ...
    results_path = os.path.join(base, "SHEAF_V02_RESULTS.md")
    spectrum_fig = os.path.join(base, "figures", "sheaf_lbd_v02_spectrum.png")
    bridges_fig = os.path.join(base, "figures", "sheaf_lbd_v02_bridges.png")
    ...
    # -----------------------------------------------------------------------
    print("EXP-RS-15  Sheaves-LBD v0.2 (bridged testbed head-to-head vs Kuramoto)")
    print("\n" + "=" * 60)
    ...
    # -----------------------------------------------------------------------
    benchmarks = [
        (os.path.join(data_dir, "research_synergy_bridged_fine_sheaf.json"), gt10_path, "Bridged fine (EXP-RS-15)"),
    ]
    ...
    for bm_corpus, bm_gt, bm_label in benchmarks:
        ...
        # EXP-RS-15: sheaf-vs-Kuramoto head-to-head (per-pair recall, mirror v07)
        _pp_recall, _pp_ranks = sheaf_perpair(bridges_20, G_corp.nodes, bm_gt)
        import json as _json
        _json.dump({"experiment": "EXP-RS-15", "method": "sheaf-frustration",
                    "corpus": os.path.basename(bm_corpus),
                    "n_communities": len(G_corp.community_ids),
                    "n_inter_community_edges": len(G_corp.edges),
                    "t2_precision_at10": {str(k): v for k, v in all_precision.items()},
                    "perpair_recall_at10": _pp_recall,
                    "perpair_ranks": [{"pair": p, "a_comm": a, "b_comm": b, "rank": rk} for p, a, b, rk in _pp_ranks],
                    "kuramoto_v07_recall_at10": 0.0},
                   open(os.path.join(base, "data", "sheaf_v02_results.json"), "w"), indent=1)
        print(f"\n  [EXP-RS-15 HEAD-TO-HEAD] sheaf per-pair recall@10 = {_pp_recall:.3f}   (Kuramoto v07 = 0.000)")
        print(f"  sheaf T2 precision@10 (n=20) = {all_precision.get(20, 0):.3f}   (Kuramoto BENCH_P10 = 0.000)")
        for p, a, b, rk in _pp_ranks:
            print(f"    {p:26} c{a}-c{b}: rank {rk if rk else '>all-bridges'}")
