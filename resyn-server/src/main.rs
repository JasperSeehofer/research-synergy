...
use resyn_core::database::queries::{AnalysisRepository, GapFindingRepository, LlmAnnotationRepository};
...
async fn main() {
    ...
    if let Err(e) = resyn_core::validation::validate_arxiv_id(&cli.paper_id) {
    ...
    let db = if let Some(ref db_endpoint) = cli.db {
        match resyn_core::database::client::connect(db_endpoint).await {
            Ok(db) => {
...
async fn run_nlp_analysis(db: &Db) {
    let extraction_repo = resyn_core::database::queries::ExtractionRepository::new(db);
    let analysis_repo = resyn_core::database::queries::AnalysisRepository::new(db);
