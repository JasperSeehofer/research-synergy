...
fn build_bridge_context(
    ...
    format!(
        ...
        analysis_a.arxiv_id,
        if open_a.is_empty() { "(none)".to_string() } else { open_a },
        analysis_c.arxiv_id,
        if open_c.is_empty() { "(none)".to_string() } else { open_c },
        shared_terms.join(", ")
    )
}
...
pub async fn find_abc_bridges(
    ...
    for i in 0..analyses.len() {
        for j in (i + 1)..analyses.len() {
            ...
            let justification = match provider.verify_gap(ABC_BRIDGE_SYSTEM_PROMPT, &context).await {
                Ok(response) => {
...
mod tests {
    ...
    fn many_shared_terms(n: usize) -> Vec<(&'static str, f32)> {
        let terms = [
            "quantum", "entanglement", "decoherence", "spin", "topological",
            "lattice", "tensor", "field", "gauge", "symmetry",
        ];
    ...
    fn test_has_direct_edge() {
        ...
        assert!(has_direct_edge(&graph, node_c, node_a), "Direction reversed should also detect edge");
    }
    ...
    async fn test_find_abc_bridges_excludes_direct_citations() {
        ...
        assert!(result.is_empty(), "Direct citations should be excluded from ABC-bridge results");
    }
    ...
    async fn test_find_abc_bridges_returns_empty_when_insufficient_shared_terms() {
        ...
        let result = find_abc_bridges(&[a, c], &[], &graph, &mut provider, true).await;
        assert!(result.is_empty(), "Fewer than 3 shared terms should not produce a bridge");
    }
    ...
    async fn test_find_abc_bridges_noop_provider_returns_empty() {
        ...
        assert!(result.is_empty(), "NoopProvider returns NO so bridge should not be confirmed");
    }
