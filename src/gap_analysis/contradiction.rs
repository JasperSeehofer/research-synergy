...
const WEAK_STRENGTHS: &[&str] = &["weak", "weak_evidence", "preliminary", "speculative", "inconclusive"];
...
fn findings_diverge(ann_a: &LlmAnnotation, ann_b: &LlmAnnotation) -> bool {
    ...
    let a_has_strong = ann_a.findings.iter().any(|f| classify_strength(&f.strength) == Some(true));
    let b_has_strong = ann_b.findings.iter().any(|f| classify_strength(&f.strength) == Some(true));
    let a_has_weak = ann_a.findings.iter().any(|f| classify_strength(&f.strength) == Some(false));
    let b_has_weak = ann_b.findings.iter().any(|f| classify_strength(&f.strength) == Some(false));
...
fn build_contradiction_context(
    ...
    format!(
        ...
        analysis_a.arxiv_id,
        if findings_a.is_empty() { "  (none)".to_string() } else { findings_a.join("\n") },
        analysis_b.arxiv_id,
        if findings_b.is_empty() { "  (none)".to_string() } else { findings_b.join("\n") },
        shared_terms.join(", ")
    )
}
...
pub async fn find_contradictions(
    ...
    for i in 0..analyses.len() {
        for j in (i + 1)..analyses.len() {
            ...
            let shared_terms = similarity::shared_high_weight_terms(&a.tfidf_vector, &b.tfidf_vector, 0.1);
            let context = build_contradiction_context(a, b, ann_a, ann_b, &shared_terms);

            let justification = match provider.verify_gap(CONTRADICTION_SYSTEM_PROMPT, &context).await {
                Ok(response) => {
...
mod tests {
    ...
    fn make_high_sim_analyses() -> (PaperAnalysis, PaperAnalysis) {
        ...
        let terms = &[("quantum", 0.8_f32), ("entanglement", 0.6), ("decoherence", 0.5)];
        (make_analysis("2301.11111", terms), make_analysis("2301.22222", terms))
    }

    #[tokio::test]
    async fn test_find_contradictions_returns_finding_for_divergent_pair() {
        let (a, b) = make_high_sim_analyses();
        let ann_a = make_annotation("2301.11111", &[("Energy gap is non-zero", "strong_evidence")]);
        let ann_b = make_annotation("2301.22222", &[("Energy gap appears negligible", "preliminary")]);
        ...
        assert!(result.is_empty(), "NoopProvider returns NO, so result should be empty");
    }
    ...
    async fn test_find_contradictions_skips_non_divergent_strengths() {
        ...
        let result = find_contradictions(&[a, b], &[ann_a, ann_b], &mut provider).await;
        assert!(result.is_empty(), "Non-divergent findings should be skipped");
    }
