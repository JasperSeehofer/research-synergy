...
mod tests {
    ...
    fn make_contradiction(paper_ids: Vec<&str>, shared_terms: Vec<&str>, justification: &str) -> GapFinding {
        GapFinding {
    ...
    fn make_abc_bridge(paper_ids: Vec<&str>, shared_terms: Vec<&str>, justification: &str) -> GapFinding {
        GapFinding {
    ...
    fn test_format_gap_table_empty_has_none_found_sections() {
        ...
        assert!(result.contains("Contradictions"), "Should have Contradictions section");
        assert!(result.contains("ABC Bridges"), "Should have ABC Bridges section");
        assert_eq!(result.matches("(none found)").count(), 2, "Both sections should show (none found)");
    }

    #[test]
    fn test_format_gap_summary_empty_has_zero_counts() {
        let result = format_gap_summary(&[]);
        assert_eq!(result, "Gap analysis: 0 contradictions, 0 ABC-bridges found across 0 papers");
    }
    ...
    fn test_format_gap_table_one_contradiction_has_headers() {
        ...
        assert!(result.contains("Papers"), "Should have Papers column header");
        assert!(result.contains("Shared Terms"), "Should have Shared Terms column header");
        assert!(result.contains("Justification"), "Should have Justification column header");
        // Paper IDs in result
        assert!(result.contains("2301.11111"), "Should contain first paper ID");
        assert!(result.contains("2301.22222"), "Should contain second paper ID");
        // Shared terms in result
        assert!(result.contains("quantum"), "Should contain shared term");
        // ABC section shows none
        assert!(result.contains("(none found)"), "ABC Bridges section should show (none found)");
    }
    ...
    fn test_format_gap_table_one_abc_bridge_has_headers() {
        ...
        assert!(result.contains("(none found)"), "Contradictions section should show (none found)");
    }
    ...
    fn test_format_gap_table_truncates_justification_at_60_chars_non_verbose() {
        ...
        assert!(long_just.len() > 60, "Test prerequisite: justification must be > 60 chars");
        ...
        assert!(result.contains("..."), "Truncated justification should end with ...");
        // Should NOT contain the full text
        assert!(!result.contains(long_just), "Full justification should not appear in non-verbose output");
    }
    ...
    fn test_format_gap_table_shows_full_justification_verbose() {
        ...
        assert!(result.contains(long_just), "Full justification should appear in verbose mode");
    }
    ...
    fn test_format_gap_summary_correct_counts() {
        ...
        assert_eq!(result, "Gap analysis: 2 contradictions, 1 ABC-bridges found across 3 papers");
    }
    ...
    fn test_format_gap_summary_counts_unique_papers() {
        ...
        let result = format_gap_summary(&findings);
        assert_eq!(result, "Gap analysis: 2 contradictions, 0 ABC-bridges found across 2 papers");
    }
