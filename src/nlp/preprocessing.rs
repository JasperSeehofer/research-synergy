...
pub fn build_stop_words() -> HashSet<String> {
    ...
    for word in &["paper", "study", "result", "show", "figure", "also", "using"] {
...
mod tests {
    ...
    fn test_tokenize_basic() {
        ...
        assert!(!tokens.contains(&"is".to_string()), "'is' should be filtered (len 2)");
    ...
    fn test_build_stop_words_contains_common() {
        ...
        assert!(stop.contains("paper"), "Missing academic boilerplate 'paper'");
        assert!(stop.contains("study"), "Missing academic boilerplate 'study'");
        assert!(stop.contains("result"), "Missing academic boilerplate 'result'");
        assert!(stop.contains("show"), "Missing academic boilerplate 'show'");
        assert!(stop.contains("figure"), "Missing academic boilerplate 'figure'");
    }
}
