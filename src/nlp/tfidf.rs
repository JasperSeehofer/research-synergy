...
mod tests {
    ...
    fn test_tfidf_corpus_relative_scores() {
        let sections1 = make_sections(Some("rare unique alpha beta gamma delta epsilon"), None, None, None, None);
        let sections2 = make_sections(Some("common alpha beta gamma delta epsilon"), None, None, None, None);
        let sections3 = make_sections(Some("common alpha beta gamma delta epsilon"), None, None, None, None);
        ...
        let doc1_tfidf = results.iter().find(|(id, _)| id == "doc1").map(|(_, v)| v).unwrap();
    ...
    fn test_abstract_only_paper_produces_scores() {
        ...
        assert!(!tfidf.is_empty(), "Abstract-only paper should produce non-empty TF-IDF");
        assert!(tfidf.values().any(|&v| v > 0.0), "Scores should be positive");
    }

    // Test 10: corpus_fingerprint same for same IDs regardless of order
    #[test]
    fn test_corpus_fingerprint_order_independent() {
        let ids1 = vec!["2301.12345".to_string(), "2301.99999".to_string(), "2302.00001".to_string()];
        let ids2 = vec!["2302.00001".to_string(), "2301.12345".to_string(), "2301.99999".to_string()];
    ...
    fn test_corpus_fingerprint_changes_on_addition() {
        ...
        let ids2 = vec!["2301.12345".to_string(), "2301.99999".to_string(), "2302.00001".to_string()];
